...
def water_color_effect(decode_array_to_img):
	water_effect_img = cv2.stylization(decode_array_to_img, sigma_s = 60.0, sigma_r = 0.6)
	status, output_img = cv2.imencode(".PNG", water_effect_img)
	return output_img
def invert_effect(decode_array_to_img):
	invert_color_img = cv2.bitwise_not(decode_array_to_img)
	status, output_img = cv2.imencode(".PNG", invert_color_img)
	return output_img
def hdr_effect(decode_array_to_img):
	hdr_img = cv2.detailEnhance(decode_array_to_img, sigma_s = 12.0, sigma_r = 0.15)
